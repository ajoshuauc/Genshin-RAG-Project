...
def extract_all_p_tags_text(mw: MediaWikiClient, title: str) -> str:
    ...
    """
    # Use MediaWiki API action=parse instead of REST API
    params = {
        "action": "parse",
        "page": title,
        "prop": "text",
...
def main():
    ...
    out_path = os.path.join(misc_dir, "shades.json")
    
    # Characters to scrape
    characters = [
        {"character": "Ronova", "url": "https://genshin-impact.fandom.com/wiki/Ronova"},
        {"character": "Naberius", "url": "https://genshin-impact.fandom.com/wiki/Naberius"},
        {"character": "Istaroth", "url": "https://genshin-impact.fandom.com/wiki/Istaroth"},
        {"character": "Asmoday", "url": "https://genshin-impact.fandom.com/wiki/Asmoday"},
    ]
    ...
    print(f"\n✅ Saved {len(output)} characters to {out_path}")
    
    # Print summary
    for item in output:
        print(f"   - {item['character']}: {len(item['text'])} chars")
